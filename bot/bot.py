...
import database
import openai_utils
from const import HELP_MESSAGE, HELP_GROUP_CHAT_MESSAGE
...
async def register_user_if_not_exists(
    ...
    if db.get_user_attribute(user.id, "current_model") is None:
        db.set_user_attribute(
            user.id, "current_model", settings.models["available_text_models"][0]
        )
...
async def message_handle(
    ...
    async def message_handle_fn():
        ...
        if use_new_dialog_timeout:
            ...
            ).seconds > settings.new_dialog_timeout and len(
                db.get_dialog_messages(user_id)
            ) > 0:
                db.start_new_dialog(user_id)
                await update.message.reply_text(
                    f"Starting new dialog due to timeout (<b>{settings.chat_modes[chat_mode]['name']}</b> mode) ✅",
        ...
        if n_first_dialog_messages_removed > 0:
            if n_first_dialog_messages_removed == 1:
                ...
                text = (
                    f"✍️ <i>Note:</i> Your current dialog is too long, so <b>{n_first_dialog_messages_removed} "
                    f"first messages</b> were removed from the context.\n Send /new command to start new dialog"
                )
            await update.message.reply_text(text, parse_mode=ParseMode.HTML)
...
async def generate_image_handle(update: Update, context: CallbackContext, message=None):
    ...
    try:
        image_urls = await openai_utils.generate_images(
            message, n_images=settings.return_n_generated_images
        )
    ...
    db.set_user_attribute(
        user_id,
        "n_generated_images",
        settings.return_n_generated_images
...
async def show_balance_handle(update: Update, context: CallbackContext):
    ...
    for model_key in sorted(n_used_tokens_dict.keys()):
        ...
        n_input_spent_dollars = settings.models["info"][model_key][
            "price_per_1000_input_tokens"
        ] * (n_input_tokens / 1000)
        n_output_spent_dollars = settings.models["info"][model_key][
    ...
    image_generation_n_spent_dollars = (
        settings.models["info"]["dalle-2"]["price_per_1_image"] * n_generated_images
    )
    ...
    # voice recognition
    voice_recognition_n_spent_dollars = settings.models["info"]["whisper"][
        "price_per_1_min"
